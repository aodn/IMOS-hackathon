...
calculate_daily_metrics <- function(basedir) {
  ...
  if (is_empty(filename) == FALSE) {
    series_dat <- read_csv(filename) %>%
      ...
      select(
        Ptt, Day, MinTempFromSeries, MeanTempFromSeries,
  ...
  # use DailyData.csv
  filename <- dir(file.path(basedir),
    pattern = "DailyData.csv",
    full.names = TRUE
  )
  ...
  out <- out %>%
    mutate(DistTravelled = track_distance_to(
      Longitude, Latitude,
      lag(Longitude),
      lag(Latitude)
    ) / 1000)
...
compute_distances <- function(path_daily, save_bathy_path) {
  ...
  # along the track
  dat.bathy <- get.depth(bat,
    x = dat$Longitude, y = dat$Latitude,
    distance = T, locator = F, res = 1
  )
  colnames(dat.bathy) <- c(
    "Longitude", "Latitude", "DistfromStart",
    "bathy_depth"
  )
...
daily_positions <- function(file_path) {
  ...
  timeZ <- tz_lookup_coords(
    lat = dat$most_likely_latitude,
  ...
  track <- dat %>%
    group_by(date) %>%
    summarise(
      ptt = unique(ptt), Latitude = mean(most_likely_latitude),
