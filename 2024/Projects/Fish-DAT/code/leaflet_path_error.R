...
colpal <- list.files(folder_path, pattern = "*colour_palette*.csv") %>% read_csv()
i = 1
...
#   function(folder_name, metadata, prob = 0.99, colpal){
    ...
    leaflet() %>%
      ...
      # Add location data:
      addPolylines(lng = pos_sf$lon, lat = pos_sf$lat, group = pos_sf$Ptt,
      ...
      addLayersControl(
        ...
        baseGroups = as.character(unique(pos_dat$Ptt)),  # add the data groups to overlay on the map
