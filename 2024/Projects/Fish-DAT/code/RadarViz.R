...
all_files <- list.files("~/Documents/GitHub/IMOS-hackathon/2024/Projects/Fish-DAT/data/outputs/",
...
for(i in 1:length(all_files)){
  # read each file
  dat <- read.csv(all_files[i], header=T); head(dat)
  ptt_id = unique(dat$Ptt); print(ptt_id)
  ...
  daily_measurements <- dat %>%
    mutate(mean_daily_distance = if_else(is.na(DistTravelled), 0, DistTravelled)) %>%
              filter(!is.na(MinTemp)) %>% mutate(min_temp = min(MinTemp)) %>%
              filter(!is.na(MeanTempPDT)) %>% mutate(mean_temp = mean(MeanTempPDT)) %>%
              filter(!is.na(MeanDepthPDT)) %>% mutate(mean_depth = mean(MeanDepthPDT))
  daily_measurements
  # compute overall summary metrics
  summary_metrics <- daily_measurements %>%
    reframe(Ptt = unique(ptt_id),
...
#out_dat
met <- left_join(met, out_dat, by=c("Ptt")); print(met)
...
for (k in 1:nrow(summary_metrics)){
  ...
  radarchart( temp_metrics  , axistype=1 , 
